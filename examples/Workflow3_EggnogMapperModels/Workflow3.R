...
#####################################R
# Preparing eggnog-mapper Annotation #

# Loading eggnog-mapper Annotation (https://github.com/eggnogdb/eggnog-mapper, from "examples/Workflow3_EggnogMapperModels/TreeseiV2_FilteredModelsv2.0.proteins.fasta")
eggnog_annotation_df <- load_eggnog_annotation("examples/Workflow3_EggnogMapperModels/TreeseiV2_FilteredModelsv2.0.emapper.annotations")

# Formatting eggnog-mapper Annotation To Match Protein IDs (If Needed)
eggnog_annotation_df$query <- sapply(strsplit(eggnog_annotation_df$query, "\\|"), `[`, 4)
eggnog_annotation_df$query <- gene2protein_id(eggnog_annotation_df$query, transcript2protein_id_df)

#############
# KOG Model #

# Building KOG Model From eggnog-mapper Annotation
kog_model_df <- create_kog_model_eggnog(eggnog_annotation_df)

############
# GO Model #

# Building GO Model From eggnog-mapper Annotation
...
# Building KEGG Pathway Model From eggnog-mapper Annotation
...
gene_ids <- read.csv("examples/Workflow3_EggnogMapperModels/gene_ids.csv")$gene_ids
