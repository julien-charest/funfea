...
#'
#' @description
#' This function loads a GTF/GFF annotation into a dataframe.
#'
...
#'
#' @description
#' This function parses a GTF/GFF annotation dataframe to generate a transcript ID to protein ID conversion dataframe.
#'
#' @param gtf_df A dataframe with GTF/GFF annotation information
#'
...
#'
#' @description
#' This function converts a vector of transcript IDs to protein IDs.
#'
...
#'
#' @description
#' This function converts a vector of gene IDs and/or gene names to protein IDs.
#'
#' @param transcript_ids A vector of gene IDs and/or gene names
#' @param transcript2protein_id_df A dataframe with gene ID, transcript ID and protein ID information
#'
#' @return A vector of protein IDs
#'
...
gene2protein_id <- function(genes, transcript2protein_id_df) {
  ...
  for (i in genes) {
    ...
    if (i %in% toupper(transcript2protein_id_df$gene_ids)) {
      protein_ids <- append(protein_ids, transcript2protein_id_df[transcript2protein_id_df$gene_ids == i,]$protein_ids)
      next
    }

    else if (i %in% toupper(transcript2protein_id_df$gene_names)) {
      protein_ids <- append(protein_ids, transcript2protein_id_df[transcript2protein_id_df$gene_names == i,]$protein_ids)
      next
    }
...
#' kog_enrichment_df <- kog_enrichment(kog_model_df, protein_ids, test = "chisq", p.adjust.method = "none")
#' @export
...
#' @param plot_type A plot type option ("bar" or "lollipop") (default = "bar")
#'
...
#' go_enrichment_df <- go_enrichment(go_model_df, protein_ids, test = "chisq", p.adjust.method = "none")
#' @export

go_enrichment <- function(go_model, protein_ids, test= "fisher", p.adjust.method= "BH"){
...
#'
#' @examples
#' kegg_annotation_df <- load_kegg_annotation(kegg_annotation_path)
#' @export

load_kegg_annotation <- function(path, transcript2protein_id_df){
  ...
  if (dim(annotation_data)[2] != 2) {
    stop("KEGG annotation file must have two columns: gene_Id,	KEGG_ko")
  }

  colnames(annotation_data) <- c("gene_Id",	"KEGG_ko")
  annotation_data$gene_Id <- sub(".*:", "", annotation_data$gene_Id)
  annotation_data$proteinId <- gene2protein_id(annotation_data$gene_Id, transcript2protein_id_df)
  annotation_data$KEGG_ko <- sub("ko:", "", annotation_data$KEGG_ko)
  annotation_data$KEGG_ko <- sub("ko", "", annotation_data$KEGG_ko)

  annotation_data <- annotation_data[,c("gene_Id", "proteinId", "KEGG_ko")]
...
#'
#' @param kegg_annotation A dataframe with KEGG annotation information
#' @param filter A filter for KEGG pathway annotation (Kingdom, Phylum, Class, Organism)(default = "All")
#'
...
#'
#' @param kegg_model 	A list of dataframes with KEGG pathway background frequencies per KEGG category (pathway_type, pathway_class, pathway_name, definition, complete)
#' @param protein_ids A vector of protein IDs
#' @param test A statistical method for testing the null of independence of rows and columns in a contingency table (default = "fisher")
#' @param p.adjust.method A statistical method to adjust p-values for multiple comparisons (default = "BH")
#'
...
#' kegg_enrichment_df <- kegg_enrichment(kegg_model_df, protein_ids, test = "chisq", p.adjust.method = "none")
#' @export
...
create_kegg_model_eggnog <- function(eggnog_annotation, filter = "All"){
  ...
  return(create_kegg_model(ec_annotation, filter = filter))
}
...
#'
#' @param enrichment_df A list of dataframes with KEGG pathway enrichment statistics per KEGG category (pathway_type, pathway_class, pathway_name, definition, complete)
#' @param pathwayType A KEGG category ("definition", "pathway_name", "pathway_class" or "pathway_type") (default = "pathway_class")
...
fetch_models <- function(strain){
  ...
  if (strain %in% available_models$strain){
    ...
    stop(sprintf("Invalid strain name. Available strain models are: '%s'", paste(models, collapse = ", ")))
  }
}
...
create_kegg_model_from_ec <- function(ec_annotation_df, filter = "All"){
  ...
  return(create_kegg_model(kegg_annotation, filter = filter))
}
